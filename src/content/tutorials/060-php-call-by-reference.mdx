---
title: "PHP - Call by Reference"
description: "Learn PHP - Call by Reference with clear explanation, code examples, and practical PHP development guide."
category: "php"
order: 600
group: "PHP Functions"
seoTitle: "PHP - Call by Reference — CodesCompiler PHP Guide"
permalink: "php-call-by-reference"
---
...
The following PHP script will help in understanding what references are. Here,$varis a normal string variable. We declare$var1as a reference to$var, append "&" symbol to the latter.
...
When we say that$var1is an alias or reference of$var, it means any change in its value will also change the value of$var, and vice versa.
...
When the function is invoked,$arg1becomes a reference to$xand$arg2becomes a reference to$y.


If, inside the function body, the value of$arg1or$arg2(or both) changes, it also causes the values of$xand$yto change.
...
The variable$nameis passed to the functionchange_name(). A reference variable&$nmbecomes its reference variable. Any change in$nmis reflected in$nameoutside the function.
...
The following code shows the example of a function returning a reference. It returns$x, which is a local static variable insidemyfunction(). Since "&" symbol is prepended to it,$a(the variable that stores the return value) becomes a reference to&x. As a result, any change in$awill also change the value of$x.
