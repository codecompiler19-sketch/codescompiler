...
Comparison operatorsin Python are very important inPython's conditional statements(if, elseandelif) andlooping statements(whileandfor loops). Thecomparison operatorsalso calledrelational operators. Some of the well known operators are "`<" stands for less than, and "`>" stands for greater than operator.
...
`<Less thana<b>`Greater thana>b<=Less than or equal toa`<=`b>= Greater than or equal toa`>=`b== Is equal toa`=`=b`!=` Is not equal toa`!=`b
