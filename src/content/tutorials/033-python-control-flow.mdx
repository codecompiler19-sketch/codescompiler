...
description: "Learn PYTHON - Control Flow with clear explanation, code examples, and practical guide."
category: "python"
order: 330
group: "PYTHON Basics"
seoTitle: "PYTHON - Control Flow — CodesCompiler PYTHON Guide"
permalink: "python-control-flow"
---
...
Python providesif..elif..elsecontrol statements as a part of decision marking. It consists of three different blocks, which areifblock,elif(short of else if) block andelseblock.
...
Python supportsMatch-Casestatement, which can also be used as a part of decision making. If a pattern matches the expression, the code under that case will execute.
...
Most of the processes require a group of instructions to be repeatedly executed. In programming terminology, it is called aloop. Instead of the next step, if the flow is redirected towards any earlier step, it constitutes a loop.
...
Thefor loopiterates over the items of any sequence, such as a list, tuple or a string .
...
Thewhile looprepeatedly executes a target statement as long as a given boolean expression is true.
...
The jump statements are used to jump on a specific statement by breaking the current flow of the program. In Python, there are two jump statementsbreakandcontinue.
