...
```php
// PHP Arrays easily mix string keys, numbers, and sub-arrays
$shoppingList = [
...
echo "`### `Shopping List Categories:";
echo "`<ul>`";

foreach ($shoppingList as $category => $items) &#123;
    if (is_array($items)) &#123;
        // Count how many items are in the sub-array natively
        echo "`<li>`**\$category** contains " . count($items) . " items.`</li>`";
    &#125; else &#123;
        echo "`<li>`**\$category:** $\$$items`</li>`";
    &#125;
&#125;
